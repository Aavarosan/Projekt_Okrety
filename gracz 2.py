...
def rysowanie_planszy(n):
    ...
    for i in range(10):
        for j in range(10):
            x = p[j][i]
            if x[0] == 0:
                pygame.draw.rect(ekran, CZARNY, (x_init + j * 10 + j * TILE_SIZE, y_init + i * 10 + i * TILE_SIZE, TILE_SIZE, TILE_SIZE))
            elif x[1] == 0:
                pygame.draw.rect(ekran, BLEKIT, (x_init + j * 10 + j * TILE_SIZE, y_init + i * 10 + i * TILE_SIZE, TILE_SIZE, TILE_SIZE))
            else:
                pygame.draw.rect(ekran, MALINOWY, (x_init + j * 10 + j * TILE_SIZE, y_init + i * 10 + i * TILE_SIZE, TILE_SIZE, TILE_SIZE))
            if x[2] == 1 and n == 1:
                text = FONT1.render('x', True, (0, 0, 0))
                textr = text.get_rect()
                textr.center = (x_init + j * 10 + j * TILE_SIZE + TILE_SIZE // 2, y_init + i * 10 + i * TILE_SIZE + TILE_SIZE // 2)
...
def get_tile(px, py, n):
    ...
    for i in range(10):
        for j in range(10):
            xp, xk, yp, yk = x_init + j * 10 + j * TILE_SIZE, x_init + (j + 1) * 10 + (j + 1) * TILE_SIZE, y_init + i * 10 + i * TILE_SIZE, y_init + (i + 1) * 10 + (i + 1) * TILE_SIZE
...
def main():
    ...
    # Ruch Przeciwnika
        ...
        while kolej_przec:
            ...
            if message != '':
                z = message.decode()
                z = [int(z[0]), int(z[1]), int(z[2])] if len(z) == 3 else [int(z[0]), int(z[1]), int(z[2])+int(z[3])]
                c = 0
